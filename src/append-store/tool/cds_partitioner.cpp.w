    ...
    for (int i = 0; i < no_partitions; ++i) 
    {
        ...
        while (creader.Next(subvec)) 
        {
            ...
            for (uint32_t jj = 0; jj < subvec.size(); ++jj) 
            {
                ...
                double readThroughput = data.size() / (latency / 1000000.0) / (1024.0*1024.0);
                std::cout << "Latency: " << latency << " size: " << data.size() << endl;
                std::cout << "  Read Throughput: " << readThroughput / (1024.0 * 1024.0) << " MBps" << endl;
                std::string shastring = tosha1(data); 
                //std::cout << PrintableSha1(shastring) << " " << data.size() << std::endl;
            }
